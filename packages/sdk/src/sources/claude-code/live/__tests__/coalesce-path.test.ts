...
const ROOT_DIR = '/tmp/claude';

describe('coalescePath (RFC 005 C5.2)', () => {
  test('task with sessionId → collapses to <rootDir>/tasks/<sid>/.coalesced', () => {
    const route: RouteResult = { category: 'task', sessionId: 's1' };
    const out = coalescePath(path.join(ROOT_DIR, 'tasks', 's1', '.lock'), route, ROOT_DIR);
    assert.equal(out, path.join(ROOT_DIR, 'tasks', 's1', TASK_COALESCE_FILENAME));
  });

  test('task event on .highwatermark coalesces to the same path as .lock', () => {
    const route: RouteResult = { category: 'task', sessionId: 's1' };
    const a = coalescePath(path.join(ROOT_DIR, 'tasks', 's1', '.lock'), route, ROOT_DIR);
    const b = coalescePath(path.join(ROOT_DIR, 'tasks', 's1', '.highwatermark'), route, ROOT_DIR);
    const c = coalescePath(path.join(ROOT_DIR, 'tasks', 's1', '7.json'), route, ROOT_DIR);
  ...
  test('task without sessionId → identity (defensive: should not happen in practice)', () => {
    ...
    const input = path.join(ROOT_DIR, 'tasks', 'whatever.txt');
    assert.equal(coalescePath(input, route, ROOT_DIR), input);
  });

  test('non-task categories → identity', () => {
    const cases: Array<[RouteResult['category'], string]> = [
      ['session', path.join(ROOT_DIR, 'projects', 'slug', 'sess.jsonl')],
      ['todo', path.join(ROOT_DIR, 'todos', 'sess-agent-a0.json')],
      ['plan', path.join(ROOT_DIR, 'plans', 'abc.md')],
      ['file_history', path.join(ROOT_DIR, 'file-history', 'sess', 'abc@v1')],
      ['settings', path.join(ROOT_DIR, 'settings.json')],
      ['settings_local', path.join(ROOT_DIR, 'settings.local.json')],
      ['session_index', path.join(ROOT_DIR, 'projects', 'slug', 'sessions-index.json')],
      ['subagent', path.join(ROOT_DIR, 'projects', 'slug', 'sess', 'subagents', 'agent-a0.jsonl')],
      ['tool_result', path.join(ROOT_DIR, 'projects', 'slug', 'sess', 'tool-results', 'abc.txt')],
      ['project_memory', path.join(ROOT_DIR, 'projects', 'slug', 'memory', 'MEMORY.md')],
    ];
    for (const [category, input] of cases) {
      const route: RouteResult = { category } as RouteResult;
      assert.equal(coalescePath(input, route, ROOT_DIR), input, `category=${category} should pass through`);
    }
  });

  test('respects a non-default rootDir', () => {
    const route: RouteResult = { category: 'task', sessionId: 's1' };
