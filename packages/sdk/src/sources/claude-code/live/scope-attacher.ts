 ...
 *
 *   - `settings` watches `rootDir` non-recursively; every other
 ...
 *     graceful `ClaudeCodeLiveUpdates.stop()` ordering.
 */

import * as path from 'node:path';

import type { Watcher, WatchEvent, Unsubscribe } from '../../../live/watcher.js';
...
/**
 * Subpath under rootDir for each attachable scope. `'.'` for
 * `settings`, which watches the rootDir root itself non-recursively
...
export interface ScopeAttacher {
   ...
   * `ClaudeCodeLiveUpdates.stop()` so unsubscribe handles land cleanly before
   ...
   * start are accepted). Called once from `ClaudeCodeLiveUpdates.start()` after
...
export function createScopeAttacher(deps: ScopeAttacherDeps): ScopeAttacher {
  ...
  async function attachScope(scope: WatchScopeKey, state: ScopeState): Promise<void> {
    ...
    const fullPath = path.join(deps.rootDir, subPath);
    // Settings watches rootDir itself non-recursively — recursive
    ...
    try {
      ...
      deps.onError(
        err instanceof Error
          ? new Error(`[ClaudeCodeLiveUpdates] failed to attach watcher on ${scope}/ (${fullPath}): ${err.message}`)
          : new Error(`[ClaudeCodeLiveUpdates] failed to attach watcher on ${scope}/ (${fullPath}): ${String(err)}`),
      );
    }
  }
