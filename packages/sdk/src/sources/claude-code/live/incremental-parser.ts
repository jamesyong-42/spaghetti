...
// Re-export shared write-batch types so Claude live consumers can import from here.
export type { ParsedRow, ParsedRowCategory } from '../../../live/parsed-row.js';
...
export function createIncrementalParser(options: CreateIncrementalParserOptions): IncrementalParser {
  ...
  async function parseTaskDelta(params: ParseFileDeltaParams): Promise<IncrementalParseResult> {
    const { path: filePath, sessionId, checkpoint, rootDir } = params;
