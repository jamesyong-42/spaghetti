...
function wrapSqlite(dbPath: string): {
  ...
  // the schema's `search_fts` virtual table shape — we don't
  // import `initializeSchema` here to keep the test narrow.
  service.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS search_fts USING fts5(content)`);
...
describe('IdleMaintenance (RFC 005 C5.1)', () => {
  test('fires wal_checkpoint + fts merge + optimize after idle window', { timeout: TEST_TIMEOUT }, async () => {
    ...
    try {
      ...
      const mergeFired = [...counts.run, ...counts.exec].some((s) => /INSERT INTO search_fts/.test(s));
  ...
  test('noteActivity() resets the idle timer', { timeout: TEST_TIMEOUT }, async () => {
    ...
    try {
      ...
      const midMerges = [...counts.run, ...counts.exec].filter((s) => /INSERT INTO search_fts/.test(s)).length;
      assert.equal(
        midMerges,
        0,
      ...
      const lateMerges = [...counts.run, ...counts.exec].filter((s) => /INSERT INTO search_fts/.test(s)).length;
  ...
  test('stop() cancels future ticks', { timeout: TEST_TIMEOUT }, async () => {
    ...
    try {
      ...
      const merges = [...counts.run, ...counts.exec].filter((s) => /INSERT INTO search_fts/.test(s)).length;
